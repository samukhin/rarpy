"""
Описание проекта:
Простой архиватор RAR 5.0, реализованный на Python 3.x. Поддерживает только режим store
(без сжатия),
...
2. Сбор списка файлов и директорий: рекурсивный обход директорий для включения
   всех вложенных файлов и поддиректорий.
3. Формирование структуры архива RAR 5.0:
   - Запись сигнатуры архива (8 байт).
    - Запись главного заголовка архива (Main Archive Header)
      с флагами и CRC.
    - Для каждой директории: запись заголовка директории
      (File Header с флагом директории, без данных).
    - Для каждого файла: запись заголовка файла (File Header) с метаданными,
      затем данные файла в режиме store.
...
- Кастомные исключения для различных типов ошибок
  (InvalidPathError, FileReadError, RarCreationError).
...
def parse_command_line_args() -> tuple[
    ...
    """Парсит аргументы командной строки и возвращает путь архива,
    список путей, флаг verbose и пароль."""
    import argparse
